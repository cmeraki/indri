...
class ImageTokenizer:
    ...
    def img_tokens_from_tensor(self, image: torch.Tensor) -> list[int]:
        image = (self.transforms(image) / 255.) * 2 - 1.0
        image = image.unsqueeze(0)
        _, _, [_, _, img_toks] = self._vq_model.encode(image)
        return img_toks

    def img_tokens_from_path(self, img_path: str) -> list[int]:
        image = read_image(img_path, mode=ImageReadMode.RGB)
        image = image.to(self._device).to(self._dtype)
        img_toks = self.img_tokens_from_tensor(image)
        return img_toks
