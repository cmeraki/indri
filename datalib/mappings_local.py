...
def audio_to_wav_bytestring(file_path):
    ...
    torchaudio.save(buffer,
                   waveform,
                   sample_rate=16000,format='mp3',encoding='PCM_S',bits_per_sample=16,backend='ffmpeg',compression=torchaudio.io.CodecConfig(bit_rate=64000))
    wav_bytestring = buffer.getvalue()
    
    return wav_bytestring
...
def prepare_local_dataset(dsname,channel_name,audio_name,folder_path,split,language):
    ...
    audio_bytes.write(audio_np.tobytes())
